...
class Session(skypy.Player):
    ...
    def local(text_name):
        return self.language.get(text_name, None) or english[text_name]
    ...
    async def test_profile(self, message):
        embed = discord.Embed(
            title='profileaccepted',
            description='correctequip?',
    ...
    async def collect_potion_level(self, message):
        try:
            ...
            await self.user.send('enternumb')
            return await self.ask_potion_level(message)

    async def ask_crit_goal(self, message):
        await self.user.send('critgoal?')
    ...
    async def ask_orbs(self, message):
        if self.over:
            self.over = False
            await self.user.send('overflux')
            self.orb_queue = {'strength': 25}
        elif self.mana:
            self.mana = False
            await self.user.send('manaflux')
            self.orb_queue = {'strength': 10}
        elif self.tuba:
            self.tuba = False
            await self.user.send('tuba')
    ...
    async def collect_enchantment_modifier(self, message):
        ...
        if message.content.title() not in activities:
            await self.user.send('pickactivities')
            return await self.ask_enchantment_modifier(message)
        ...
        await client.log('enchantmentmodifier')
    ...
    async def calculate_optimal_talismans(self, message):
        ...
        await self.user.send('algstarted')
